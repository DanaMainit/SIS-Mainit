...
root.title("Student Information System - DANA MAINIT")
root.resizable(0,0)
        ...
        with open('data.csv','r') as csv_file:
                ...
                for line in read:
                        ...
                        if(line['id#'].startswith(searchword) or (line['name'].lower()).startswith(searchword.lower())):
                                ...
                                currFrame = Frame(myFrame, bg="#F4CFDF", highlightbackground="#B6D8F2", highlightthickness=3, height=100, width=470)
                                currFrame.grid(row=i, column=0, padx=5, pady=5)
                                currFrame.propagate(0)

                                picFrame = Label(currFrame, image=pic, height=90, width=90, bg="#F4CFDF")
                                picFrame.pack(side=LEFT, padx=(7,0), pady=7)
                                picFrame.image=pic
                                    
                                textFrame = Frame(currFrame, height=90, width=360, bg="#F4CFDF")
                                ...
                                info = Label(textFrame, text=" ID#\t: "+line['id#']+
                                                ...
                                                "\nCOURSE\t: "+line['course']+
                                                "\nYEAR\t: "+line['year']+
                                                "\nGENDER\t: "+line['gender'],justify=LEFT, bg="#F4CFDF",anchor="w")
                                info.pack(side=LEFT)
                                
                                thisFrame = Frame(textFrame,bg="#F4CFDF")
                                thisFrame.pack(side=RIGHT)
                                #Creates an instance of the line['id#'] variable so that each button will have different values
                                delete = Button(thisFrame, image=button_delete, borderwidth=0,bg='#F4CFDF', command=lambda x=line['id#']:deleteStudent(x))
                                ...
                                edit = Button(thisFrame, image=button_update, borderwidth=0,bg='#F4CFDF', command=lambda x=line:info_window("edit",x))
...
def info_window(command,student):
        ...
        infoWindow = Toplevel()
        infoWindow.configure(bg="#F4CFDF")
        ...
        tempFrame = Frame(thisFrame,bg="#F4CFDF")
        tempFrame.grid(row=5, column=0, columnspan=2)
        cancel = Button(tempFrame, image=button_cancel, borderwidth=0,bg='#F4CFDF', command=infoWindow.destroy)
        cancel.image=button_cancel
        cancel.grid(row=0,column=0, padx=5, pady=5)
        add = Button(tempFrame, image=button_add if command=="add" else button_save, borderwidth=0,bg='#F4CFDF', command=addStudent)
        add.image=button_add if command=="add" else button_save
        add.grid(row=0,column=1, padx=5, pady=5)

#Widgets in the main window                    
header = Frame(root, height=100, width=500, bg="#B6D8F2")
header.propagate(0)
header.grid(row=0, column=0)
Label(header, text="STUDENT INFORMATION SYSTEM", font = 'Helvitica 20 bold', bg="#B6D8F2", fg="white").place(relx=.5, rely=.4, anchor='c')
Label(header, text="a simple demonstration", font = 'Helvitica 12 italic', bg="#B6D8F2", fg="white").place(relx=.5, rely=.65, anchor='c')

#Code for a dynamic searchbar
searchFrame = Frame(root)
searchFrame.grid(row=1,column=0,pady=3)
Label(searchFrame, bg="#F4CFDF", text="SEARCH:", anchor=W, font = 'Arial 9 bold', fg="white").grid(row=0, column=0)
